...
一个用于 AstrBot 的生日提醒插件。插件会读取 `birthdays.txt`，每天在配置时间检查当天生日，并向每条记录指定的群聊发送祝福。
...
到点后，插件会读取 `birthdays.txt`，筛选当天 `MM-DD` 等于 `05-03` 的记录。如果发现张三生日，就提取该记录目标群当前生效的人设 prompt，连同生日信息交给模型生成祝福，并主动发送到这一行配置的群 `12345678`。发送成功后会写入 `sent_state.json`，避免当天重复发送。

它不是为每个人单独注册一个未来定时器，而是插件内部维护一个“每天一次”的检查循环：每次到点检查今天有哪些生日，再按群分组发送。

## 生日文件格式

编辑插件目录下的 `birthdays.txt`，每行一个人物、生日和群号，三列用空格分隔：
...
- `birthdays.txt` 是本地数据文件；如果你要公开 fork/发布插件，请只保留注释示例，不要提交真实名单。
